...
- 多 Codex 会话协作时，先确认身份和当前待办；如果用户指定身份（如若命、听云、清秋、观止、霜弦），按 `docs/codex-collaboration-roles.md` 的 `agentKey`、职责边界、交接格式和验证要求工作。
...
- `docs/codex-collaboration-roles.md` 只在首次进入项目、身份/协作规约不确定、规则变化或用户明确要求时补读；`docs/codex-cold-start.md` 只在完整冷启动、环境不熟、长时间缺席或复杂 handoff 时补读。
