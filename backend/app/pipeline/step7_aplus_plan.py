...
def _diagnosis_text(value, max_length: int, fallback: str) -> str:
    return _trim_text(value, max_length, fallback)


def _diagnosis_choice(value, allowed: set[str], fallback: str) -> str:
    text = _compact_text(value).lower().replace("-", "_").replace(" ", "_")
    return text if text in allowed else fallback


def _story_type_from_product(pd: ProductData, selling_points: list) -> str:
    category = _compact_text(getattr(pd, "leaf_category", None) or getattr(pd, "amazon_category", None)).lower()
    text = " ".join([category, _compact_text(getattr(pd, "listing_title", None) or getattr(pd, "title", None)), " ".join(map(str, selling_points))]).lower()
...
def _evidence_strength(pi: ProductImage | None, selling_points: list) -> str:
    reference_count = _reference_candidate_count(pi)
...
def _diagnosis_module_strategy(raw_strategy, story_type: str) -> dict:
...
def _normalize_product_narrative_diagnosis(
    raw_value,
    *,
    product: Product,
    ...
    diagnosis = {
        ...
        "differentiation_angle": _diagnosis_text(
            ...
            260,
            f"Differentiate through supported use context and visible details rather than unsupported claims.",
        ),
...
def _build_enhanced_basic_aplus_plan(raw_plan: dict, *, product: Product, product_data: ProductData, product_image: ProductImage | None, selling_points: list) -> dict:
    ...
    plan = {
        ...
        "product_narrative_diagnosis": _normalize_product_narrative_diagnosis(
...
def _build_standard_header_image_text_plan(
    ...
    *,
    ...
    plan = dict(raw_plan)
    plan["product_narrative_diagnosis"] = _normalize_product_narrative_diagnosis(
        raw_plan.get("product_narrative_diagnosis") or raw_plan.get("narrative_diagnosis"),
        product=product,
        product_data=product_data,
        product_image=product_image,
