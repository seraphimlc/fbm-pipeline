...
API 配置读取顺序：
1. 默认模板：`scripts/config.json`（不放密钥）
2. 应用配置：`backend/.env`
3. 本地私有文件：`scripts/config.local.json`（不提交）
4. 环境变量：`GPT_IMAGE_API_KEY`、`GPT_IMAGE_API_BASE`、`GPT_IMAGE_MODEL`、`GPT_IMAGE_DEFAULT_SIZE`
