...
from app.services.giga_price_sync import GigaPriceSyncOptions, sync_giga_price_snapshot  # noqa: E402


def _default_batch_id(site: str) -> str:
    return f"{datetime.now().strftime('%Y%m%d')}-{site.lower()}-price"


async def main() -> None:
    parser = argparse.ArgumentParser(description="Sync GIGA price snapshot into the local FBM database.")
    ...
    parser.add_argument("--batch-id", help="Price snapshot batch id. Defaults to YYYYMMDD-{site}-price.")
    parser.add_argument("--task-id", default="daily-giga-price", help="Optional task id for logs.")
    ...
    print({
        ...
        "price_changed_count": result.price_changed_count,
