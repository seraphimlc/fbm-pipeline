...
- Lingxing A+ 发布工程线 T1 只建立数据/状态基础：`backend/app/aplus_publish/status.py` 定义发布状态 registry，`backend/app/services/aplus_publish_state.py` 统一写 Product/Catalog 发布状态镜像和 AplusUploadItem 外部证据，`backend/app/database.py` 补齐字段和索引。当前尚未注册 `lingxing_listing_sync`、`lingxing_aplus_publish`、`lingxing_aplus_draft_visibility` 或 `lingxing_aplus_submit` task type，也没有新 planner/worker/API；T2+ 才能引入任务中心执行链路。
...
- Lingxing A+ 发布 task 迁移问题：T1 先看 `backend/app/aplus_publish/status.py`、`backend/app/services/aplus_publish_state.py`、`backend/app/models/models.py` 和 `backend/app/database.py`；如果要找 planner/worker/API，当前应确认尚未存在，后续 T2+ 才允许新增。
