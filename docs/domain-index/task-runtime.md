...
- 商品域 ProductTaskAction 当前包含 `product_material_prepare`、`product_auto_image_selection`、`product_competitor_search`、`product_competitor_visual_match`、`product_competitor_candidate_capture`、`product_auto_competitor_selection`、`product_keyword_research`、`product_image_analysis`、`product_customer_mindset`、`product_listing_generation`。新建 GIGA 商品的自动链路从 `product_material_prepare -> product_auto_image_selection -> product_competitor_search -> product_competitor_visual_match` 自动续接；成功 hook 必须记录下游 TaskRun IDs，创建失败投影到对应 workflow failed，不能停在 pending 冒充等待人工。内容生成尾段固定为 `product_image_analysis -> product_customer_mindset -> product_listing_generation`。心智 action 使用 `backend/app/pipeline/customer_mindset.py`，同时消费 GIGA snapshot、Information 素材事实和图片分析证据，回答 13 个固定核心问题，并按商品证据生成 2 至 5 个动态问题，总计 15 至 18 题；成功落库后才创建 Listing run。Listing 与 A+规划读取同一简报；Listing success 仍通过 `_project_listing_completed()` 投影到 export-ready，`pipeline_target=aplus_done` 再继续独立 A+ task，A+失败不得回滚 export-ready。
