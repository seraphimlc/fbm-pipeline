...
- 运行中的 step 会按租约心跳续期；网络超时、断连、限流、5xx 和 MySQL 连接丢失等可恢复基础设施异常会在该 step 的 `max_attempts` 预算内自动重试。低置信度、缺配置、数据/校验错误仍 fail-closed 并保留给人工处理。
- A+ worker 在重试/重启后会复用已持久化且五模块位置完整的 Step 7 规划和 Step 8 脚本；只有缺失或不完整的阶段才重新调用模型。Step 9 若五张图片均已落库且本地可读，会把收尾传输断连收敛为成功，避免重复付费出图。
- 自动选图会将同一商品的 Contact Sheet 批次以受限并发执行（默认 2），保持每批完整结果校验与全局合并顺序；供应商或模型限流时可将 `AUTO_IMAGE_SELECTION_VLM_CONCURRENCY` 设为 1。
...
- 商品域 ProductTaskAction 当前包含 `product_material_prepare`、`product_auto_image_selection`、`product_competitor_search`、`product_competitor_visual_match`、`product_competitor_candidate_capture`、`product_auto_competitor_selection`、`product_keyword_research`、`product_image_analysis`、`product_customer_mindset`、`product_listing_generation`。心智任务回答 13 个固定核心问题，并按商品证据生成 2 至 5 个动态问题，总计 15 至 18 题。内容生成尾段为 `product_image_analysis -> product_customer_mindset -> product_listing_generation -> generate_aplus`；A+ worker 完成五张已持久化图片后把商品投影到 `confirm_images_aplus/pending`。只有详情页确认 API 才能把它写入 `flow_done/succeeded` 和 `CatalogProduct.confirmed_at`，因此导出任务只能看到已确认商品；A+ 失败保持不可导出。
...
- 素材准备 planner：`backend/app/task_planners/product_material_prepare.py`
...
- 自动启动：planner/action 创建 task run 时若 `auto_start=True`，首个可执行 step 持久化为 `ready`，提交后调用 `kick_task_runtime()`；runner 在当前服务进程内领取 ready step，不需要用户点击 wake。整个 drain 同时受进程内锁和 MySQL 命名租约保护，重启/重复 kick 不能并发领取不同 step；若准确命名锁的 holder 已 Sleep 超过普通 step 租约、且全库没有 running step，runner 会回收该失效连接后再领取，绝不触碰活跃 worker。wake 仅用于诊断态/手动恢复，不是新 run 正常执行路径。
...
- A+ 自动触发 A1/A2：先看 `backend/app/services/aplus_auto_trigger.py`、`backend/app/product_tasks/actions.py` 的 Listing success hook、`backend/app/task_planners/aplus_generate.py` 和 `scripts/test_aplus_auto_trigger_a1_a2.py --stage a2`；A2 默认开启，通过新任务中心 `aplus_generate` 创建/复用单品 A+ run，不使用旧 `offline_tasks`；已被取代的 active run 仅作审计，不可阻断 A+。
