...
- Step 10 的 in-session 路径先完成模型、图片、OSS 和 workbook 等慢操作，再 flush 元数据；`build_catalog_export_zip()` 用每行 savepoint 隔离业务失败，并在每行结束后立即提交该行的 UPC/模板元数据，避免 SQLite 写锁跨越下一行的外部调用。任务开始进度也在进入 builder 前提交，使独立连接上的租约心跳可持续续期。CatalogProduct 的最终导出事实与任务终态仍由 scheduler 在文件上传成功后一次提交；旧同步 API 的末尾 commit 只负责其调用侧尚未提交的最终状态。
...
- 项目规则校验：普通静态/非 DB 段可用 `make test-project-rules`；完整 R1 DB gate 固定用 `python3 scripts/testing/run_with_r1_sqlite.py -- make test-project-rules`。只有该字面 argv 会被改写为固定 system make + repo Makefile 并获得 marker secrets；PATH/绝对/相对 fake make 均不能伪造 VERIFIED。
- Catalog export outcome/MySQL：`cd backend && .venv/bin/python ../scripts/test_stability_repair_r1_catalog_export.py`
- Catalog export 前端真实 API/Chromium：`backend/.venv/bin/python scripts/test_stability_repair_r1_catalog_frontend.py`；脚本建立并 finally 删除专用 `fbm_pipeline_r1_*`，启动真实 FastAPI/Vite，不使用 route/HAR/API mock。
