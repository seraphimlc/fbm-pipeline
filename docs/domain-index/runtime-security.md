...
- 本地一键启动 `scripts/start.sh` 在启动 uvicorn 前会显式执行 `python -m app.database`，跑可重复 schema maintenance，确保 ORM 新字段和 MySQL 现有测试库对齐；`DATABASE_BACKEND=sqlite` 时首次启动本地文件库也会自动建表。后端进程不支持热切库，切换后必须重启。
