...
- A+ Step7 默认仍走旧 `standard_header_image_text_v1` 五张横幅范式。Step7 的 LLM 规划必须先输出顶层 `product_narrative_diagnosis`（商品叙事诊断），包括购买动机、使用场景、主要疑虑、证据强度/缺口、差异化角度、各横幅叙事任务和禁用 claim；本地逻辑只做归一化和保守兜底。Step8 必须把该诊断连同五个模块一起传给脚本 LLM，并让每张横幅说明如何使用诊断，避免生成可套任意商品的泛化 A+ 图。
