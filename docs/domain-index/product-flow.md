...
- 自动选竞品成功后，商品必须先进入 `keyword_research` 商品准备任务。该任务依次执行关键词反查（卖家精灵无结果才 LLM 兜底）、建议售价计算和 Amazon 类目匹配；只有 `ProductData.keywords_top`、`suggested_price` 和 `leaf_category` 都已落库，才创建图片分析任务。图片分析和 Listing 的前置校验也要求这三项输入，不能把问题拖到导出阶段。
- Amazon 图片分析 / 用户心智梳理 / Listing 生成：`product_image_analysis` success 只创建或复用 `product_customer_mindset` run，不得直接创建 Listing。心智任务基于商品事实、关键词、当前选中竞品和 `ProductImage.image_analysis` 回答 13 个固定核心问题，并按商品证据从实际使用行为、材质或性能参数、摆放或环境适配、长期使用或维护、买错风险或证据缺口等视角择优生成 2 至 5 个商品特有、高决策影响且互不重复的动态问题，总计 15 至 18 题；逐题保存证据引用、置信度、未知项和内容用途，并将统一策略简报写入 `ProductData.customer_mindset`。只有该产物落库后才创建或复用 `product_listing_generation`。Listing 与 A+规划消费同一简报，避免标题、五点和后续视觉叙事互相偏离。任一直接 Step 6、重试、继续或批量入口都必须在缺少心智产物时先补心智任务，不能绕过到 Listing。重复 success 复用 active 下游 run，已 `flow_done/succeeded` / `Product.status=completed` 的商品 no-op。`product_listing_generation` success 仍是唯一进入 `flow_done/succeeded`、`Product.status=completed`、商品列表 `export_ready/待导出` 的主流程入口；failure/cancel/interrupted 不得写 completed。保护门继续阻断真实 ASIN、导出历史、Amazon 模板输出、A+ 上传证据和预先存在的 `CatalogProduct.confirmed_at`。
...
- 用户心智梳理：`backend/app/pipeline/customer_mindset.py`, `backend/app/task_planners/product_customer_mindset.py`, `backend/app/product_tasks/actions.py`, `ProductData.customer_mindset`
...
- 图片分析/心智梳理/Listing：图片分析 success hook -> `product_customer_mindset` -> `ProductData.customer_mindset` -> `product_listing_generation` -> Listing success hook -> `_project_listing_completed()` -> `flow_done/succeeded` / `Product.status=completed` / 商品列表 `export_ready`。心智失败停在 `customer_mindset/failed` 并通过任务中心查看或重试，不能伪装成 Listing 失败。Listing commit 后仍可按默认关闭配置 best-effort 触发 A+ 派生任务。`retry_image_analysis`、`retry_listing_generation` 继续使用后端安全入口；前端不自行推业务状态。
...
- TikTok 隔离 MySQL/API：`R1_TEST_MYSQL_ADMIN_URL='mysql+asyncmy://root@127.0.0.1:3306/' backend/.venv/bin/python scripts/test_stability_repair_r1_tiktok.py`
- TikTok 真实 API 页面：`R1_TEST_MYSQL_ADMIN_URL='mysql+asyncmy://root@127.0.0.1:3306/' backend/.venv/bin/python scripts/test_stability_repair_r1_tiktok_frontend.py`
...
- Listing 短标题、独立商品亮点、旧五点与真实模板列契约：`cd backend && .venv/bin/python ../scripts/test_listing_title_highlights.py`。自动生成五点另有固定的买家决策职责（购买理由、已证实体验、场景、适配实用性、购买边界），目标每条不超过 320 字符；要求内部审计记录具体事实和用户心智证据，拒绝空泛营销词和高度重复的条目。
