...
- 本地路径：`/Users/jiyuhang/code/fbm-pipeline`
...
后端虚拟环境通常已经在 `backend/.venv`。如果不存在：

```bash
cd /Users/jiyuhang/code/fbm-pipeline/backend
python3 -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
```

前端依赖通常已经在 `frontend/node_modules`。如果不存在：

```bash
cd /Users/jiyuhang/code/fbm-pipeline/frontend
npm install
```

环境变量模板在 `backend/.env.example`。真实 `backend/.env` 不应提交到 GitHub。
...
```bash
cd /Users/jiyuhang/code/fbm-pipeline
./scripts/start.sh
```
