...
第一版先接入“商品中心拉取大健商品”和“拉品后图片下载”链路，并预留 Excel 导出、Amazon Listing 抓取、A+ 生成等任务类型。
...
- GIGA 主数据拉取完成后，自动继续执行该 batch 的图片下载步骤。
- 任务中心页面展示任务列表和步骤详情。
...
5. 主数据成功后创建并执行同一 batch 的 `giga_image_download` 步骤。
6. 每个步骤完成后刷新父任务统计。
7. 商品中心刷新商品数据；任务中心展示离线任务进度。
