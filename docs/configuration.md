...
状态：当前操作参考；配置键与默认值仍以 `backend/app/config.py`、`backend/.env.example` 和启动脚本为准。
...
GIGA 拉品使用 Open API，不依赖 Chrome 页面爬取。店铺 API 地址、AK/SK、站点和履约方式只在「店铺维护」里配置，不写入全局 `backend/.env`。全局仅保留同步分页大小：

```env
GIGA_SYNC_PAGE_SIZE=200
```

同步命令、API、数据表和业务口径统一见 `docs/runbook.md` 与 `docs/domain-index/data-sources.md`；本文件不重复维护操作步骤。
